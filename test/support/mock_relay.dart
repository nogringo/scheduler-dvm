// Vendored from package:ndk 0.8.4-dev.1 test/mocks/mock_relay.dart.
// Adapted for scheduler_dvm tests with public stored events and generic tag
// filtering.
...
class MockRelay {
  ...
  Set<Nip01Event> get storedEvents => _storedEvents;
  ...
  bool _eventMatchesFilter(Nip01Event event, Filter filter) {
    ...
    if (!_matchesGenericTagFilters(event, filter)) {
      return false;
    }
  ...
  bool _matchesGenericTagFilters(Nip01Event event, Filter filter) {
    ...
    for (final entry in tags.entries) {
      final tagName = entry.key.startsWith('#')
          ...
          : entry.key;
      final eventTagValues = event.tags
          .where((tag) => tag.length > 1 && tag.first == tagName)
          .map((tag) => tag[1])
          .toList();
      if (!entry.value.any(eventTagValues.contains)) {
        return false;
      }
    }
    return true;
